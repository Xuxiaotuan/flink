...
public class SinkReuseITCase extends AbstractTestBase {
    ...
    public void testAnonymousCollectSink() throws Exception {
        ...
        try (CloseableIterator<Row> rows =
                tEnv.sqlQuery("SELECT * FROM sinkReuseItCaseSource1").execute().collect()) {
            assertThat(rows)
                    .toIterable()
    ...
    public void testDistinctSinksWithSameSummaryName() throws Exception {
        ...
        statementSet.addInsertSql("INSERT INTO `db.sink` SELECT * FROM sinkReuseItCaseSource1");
        statementSet.addInsertSql(
                "INSERT INTO `default_database.db`.`sink` SELECT * FROM sinkReuseItCaseSource2");
